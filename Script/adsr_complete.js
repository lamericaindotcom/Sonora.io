/* =====================================================================
   ADSR.JS - ENVELOPE & POLYPHONIC VOICE MANAGEMENT (FIXED)
   ===================================================================== 
   ...
   Usage:
   - Inclure dans index.html AVANT oscillateur.js
...
class ADSRManager {
  ...
  noteOn(midi, freq) {
    ...
    console.log(`🎵 Note ON: MIDI ${midi} - Active voices: ${this.getActiveVoiceCount()}`);
  }
  ...
  noteOff(midi) {
    ...
    if (voice) {
      ...
      this.heldNotes.delete(midi);
      console.log(`🔇 Note OFF: MIDI ${midi} - Active voices: ${this.getActiveVoiceCount()}`);
    }
  }

  /**
   * ✅ FIXE: Met à jour les paramètres ADSR (CRITIQUES!)
   * Cette méthode est appelée chaque fois qu'un slider ADSR change
   */
  setADSR(attack, decay, sustain, release) {
    ...
    console.log('⚙️ ADSR Parameters Updated:', {
      attack: (this.adsr.attack * 1000).toFixed(0) + 'ms',
      decay: (this.adsr.decay * 1000).toFixed(0) + 'ms',
      sustain: (this.adsr.sustain * 100).toFixed(0) + '%',
      release: (this.adsr.release * 1000).toFixed(0) + 'ms'
    });
  }
...
console.log('✅ adsr.js loaded - ADSR envelope manager ready');