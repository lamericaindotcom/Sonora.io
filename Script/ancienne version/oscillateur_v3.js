/* =====================================================================
   OSCILLATEUR.JS - SYNTHESIZER ENGINE WITH ADSR POLYPHONY (CORRECTED)
   ===================================================================== 
   ...
   - Filtre Biquad (lowpass) avec bypass dynamique
   - 4 voix polyphoniques ADSR
   - Clavier virtuel (souris) + PC (AZERTY)
   - Oscilloscopes en temps réel (1 pour osc1, 1 pour osc2)
...
/* =====================================================================
   AUDIO SETUP - ROUTING & NODES
===================================================================== */

const AudioContext = window.AudioContext || window.webkitAudioContext;
const audioCtx = new AudioContext({ latencyHint: 'interactive' });

// ===== ANALYSERS POUR LES OSCILLOSCOPES =====
...
// ===== BYPASS SPLITTER =====
// Créer une structure de routing pour gérer le bypass dynamiquement
const dryGain = audioCtx.createGain();      // Bypass direct
const wetGain = audioCtx.createGain();      // Via le filtre
dryGain.gain.value = 0;  // Disabled by default
...
// ===== OUTPUT NODE POUR LE ADSR MANAGER =====
// Crée un gain node qui décide entre filtre et bypass
...
// Passe les analyseurs pour que les voix les alimentent
let adsr = new ADSRManager(audioCtx, masterOutput, 4);
adsr.setAnalysers(analyser1, analyser2);
console.log('✅ ADSRManager initialized with analysers');
...
window.addEventListener('load', () => {
  ...
  if (filterBypass) {
    filterBypass.addEventListener('change', (e) => {
      ...
      // Routing dynamique : bypass ou filtre
      if (filterBypassed) {
        dryGain.gain.value = 1;   // Enable bypass
        wetGain.gain.value = 0;   // Disable filter
        console.log('🚫 Filter bypass: ENABLED (signal goes directly to output)');
      } else {
        dryGain.gain.value = 0;   // Disable bypass
        wetGain.gain.value = 1;   // Enable filter
        console.log('✓ Filter bypass: DISABLED (signal goes through filter)');
      }
    });
  }
...
console.log('✅ oscillateur.js loaded - Full ADSR polyphony enabled!');
