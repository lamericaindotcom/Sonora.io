/* =====================================================================
   OSCILLATEUR.JS - SYNTHESIZER ENGINE WITH ADSR POLYPHONY (OPTIMIZED)
   ===================================================================== 
   
   Dépendances : adsr.js → adsr_extension_v2.js → oscillateur_final.js
   ...
   - Oscilloscopes intelligents (auto-start/stop)
...
// Initialize ADSR Manager
let adsr = new ADSRManager(audioCtx, masterOutput, 4);
adsr.setAnalysers(analyser1, analyser2);
console.log('✅ ADSRManager initialized with smart scope monitoring');
...
function triggerNoteOn(midi) {
  ...
  // Les scopes sont gérés automatiquement par startScopeMonitor()
}

function triggerNoteOff(midi) {
  adsr.noteOff(midi);
  // Les scopes s'arrêtent automatiquement quand 0 voix actives
}
...
window.addEventListener('load', () => {
  ...
  if (filterBypass) {
    filterBypass.addEventListener('change', (e) => {
      ...
      if (filterBypassed) {
        ...
        dryGain.gain.value = 0;
        wetGain.gain.value = 1;
        console.log('✓ Filter bypass: DISABLED');
      }
    });
  }
  ...
  initUI();
  console.log('✅ UI initialized - Full ADSR with smart scopes ready!');
});
...
console.log('✅ oscillateur.js loaded - Optimized polyphony + ADSR visualization!');
